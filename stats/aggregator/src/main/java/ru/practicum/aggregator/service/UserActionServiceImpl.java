...
public class UserActionServiceImpl implements UserActionService {
    ...
    public List<EventSimilarityAvro> calculateSimilarity(UserActionAvro userActionAvro) {
        ...
        if (diff > 0) {
            ...
            log.info("Рассчитываем схожесть с {}", anotherEvents.size());
    ...
    private Double updateEventAction(UserActionAvro userActionAvro) {
        Long eventId = userActionAvro.getEventId();
        Long userId = userActionAvro.getUserId();
        Double result = getActionType(userActionAvro.getActionType());
        ...
        if (oldWeight == null || result > oldWeight) {
            ...
            return oldWeight == null ? result : result - oldWeight;
        }

        return 0.0;
    }

    private Double getActionType(ActionTypeAvro actionTypeAvro) {
